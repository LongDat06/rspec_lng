...
server '10.68.11.128', user: 'guardian', roles: %w(web db app)

set :rails_env,       'staging'
set :branch,          ENV['BRANCH']
set :stage,           'staging'
set :deploy_to,       "/opt/#{fetch(:application)}"
set :puma_bind,       "unix:/tmp/#{fetch(:application)}_puma.sock"
...
set :linked_files,    %w{config/database.yml config/mongoid.yml config/newrelic.yml}
