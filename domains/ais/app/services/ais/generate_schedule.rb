module Ais
  class GenerateSchedule
    ...
    def valid_interval?(start_in)
      start_in.present? && 10.minutes.ago < min_to_time(start_in) && min_to_time(start_in) < Time.zone.now.end_of_day
    end
    ...
    def jobs
      @jobs ||= YAML.load_file(Rails.root.join('domains/ais/config/schedule', "#{Rails.env}.yml"))
    end
  end
end
